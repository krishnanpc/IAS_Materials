function [MeanPredator, MeanPrey]=PredatorPreySimpleWithGraphs(WidthOfMatrix,NumberOfIterations)
...
while counter<=NumberOfIterations
    ...
    %=20
    Predators = Predators|(Preys & (PredatorsIndecies>=1) & (rand(WidthOfMatrix)>0.2));  % Räuber vermehren =    sich
    Preys = Preys - ((Predators & Preys));                   % auf Kosten der Beute
    Predators = Predators-(Predators & (PreysIndecies==0)&(rand(WidthOfMatrix)>0.75));   % sonst sterben =    Räuber
    Preys = (Preys==1)|((Preys==0) & (rand(WidthOfMatrix)>0.995));% Beute vermehrt =    sich    =20
